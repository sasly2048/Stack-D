...
export function AtlasWhisper({
  context = "dashboard",
}: {
  context?: "dashboard" | "insights" | "post-session";
}) {
  ...
  useEffect(() => {
    if (rec && visible && !shownRef.current) {
      ...
      track("atlas.recommendation_shown", {
        context,
        topic: rec.topic,
  ...
  const dismiss = () => {
    try {
      localStorage.setItem(DISMISS_KEY, String(Date.now() + 1000 * 60 * 60 * 6));
    } catch {
      /* ignore */
    }
  ...
  const opener =
    context === "post-session"
      ? "Well held. Here's what I'd try next —"
      : context === "insights"
        ? "A pattern I noticed —"
        : "Atlas here.";
