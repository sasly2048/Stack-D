...
export function MatrixText({
  ...
  return (
    // `aria-label` is prohibited on a generic <span>, so the readable text is
    // exposed as a visually-hidden copy and the animated glyphs are hidden.
    <span ref={ref} className={className}>
      ...
      <span aria-hidden="true" className="tabular-nums">
        {chars.map((c, i) => (
          <span
            key={i}
            style={{ display: "inline-block", minWidth: c === "" ? "0.5em" : undefined }}
          >
            {c || "\u00A0"}
          </span>
        ))}
