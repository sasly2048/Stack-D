          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="text-xs">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, visibility: e.target.value as "open" | "request" | "invite" })
                }
              >
        ...
        <div className="space-y-3">
          ...
          {meta.pinned_message && (
            <div className="border-l-2 border-ember pl-3 py-1 bg-ember/5 rounded-r">
              <div className="text-[9px] font-mono uppercase tracking-[0.3em] text-ember mb-0.5">
                PINNED
              </div>
          ...
          {stats && stats.goal_seconds && stats.goal_seconds > 0 && (
            <div>
              <div className="flex justify-between text-[10px] font-mono uppercase tracking-widest text-muted-foreground mb-1">
                ...
                <span>
                  {fmtHours(stats.focus_seconds_total)} / {fmtHours(stats.goal_seconds)}
                </span>
              </div>
          ...
          <div className="flex gap-4 text-[10px] font-mono uppercase tracking-widest text-muted-foreground pt-1">
            <span>
              Visibility · <span className="text-silver">{meta.visibility}</span>
            </span>
            {mods.length > 0 && (
              <span>
                Mods · <span className="text-silver">{mods.length}</span>
              </span>
            )}
          </div>
        </div>
...
export function JoinRequestsPanel({
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const load = () =>
      fetchReqs({ data: { roomId } })
        .then((r) => {
          if (mounted) setReqs(r);
        })
        .catch(() => {});
    load();
    const ch = supabase
