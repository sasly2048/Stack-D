...
export function DynamicGreeting() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: profile }, { data: hist }, { data: friends }, { data: cp }] =
        await Promise.all([
          supabase
            .from("profiles")
            .select("display_name,current_focus_streak")
            .eq("id", uid)
            .maybeSingle(),
          supabase
            .from("focus_history")
          ...
          supabase
            ...
            .select("progress,challenge:challenges(target)")
            .eq("user_id", uid)
      ...
      const cpRow = (cp ?? [])[0] as
        | { progress: number; challenge: { target: number } | null }
        | undefined;
      const challengeProgress = cpRow?.challenge?.target
        ? Math.min(1, cpRow.progress / cpRow.challenge.target)
        : 0;
      setCtx({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.04] to-transparent p-6 md:p-8">
      <div
        ...
        style={{
          background:
            "radial-gradient(600px 200px at 100% 0%, rgba(240,169,104,0.15), transparent)",
        }}
      />
      <div className="relative">
        ...
        {ctx && (
          <ul className="mt-6 grid gap-2 md:grid-cols-2 text-sm text-muted-foreground">
            {ctx.yesterdaySeconds > 0 && (
              <li>
                ...
                <span className="text-silver font-semibold">
                  {Math.round((ctx.yesterdaySeconds / 3600) * 10) / 10}h
                </span>{" "}
                yesterday.
              </li>
            )}
            {ctx.friendsOnline > 0 && (
              <li>
                · <span className="text-silver font-semibold">{ctx.friendsOnline}</span>{" "}
                {ctx.friendsOnline === 1 ? "friend is" : "friends are"} already focusing.
              </li>
            )}
            {ctx.challengeProgress > 0 && ctx.challengeProgress < 1 && (
              <li>
                ...
                <span className="text-silver font-semibold">
                  {Math.round(ctx.challengeProgress * 100)}%
                </span>{" "}
                done.
              </li>
            )}
            {ctx.streak > 0 && (
              <li>
                · Current streak: <span className="text-ember font-semibold">🔥 {ctx.streak}</span>
              </li>
            )}
          </ul>
        )}
