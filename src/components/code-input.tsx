  ...
  return (
    ...
    >
      <span id="code-input-hint" className="sr-only">
        Enter a 6-character room code. You can paste a code or type one character at a time. Use
        backspace to delete; arrow keys to move between characters.
      </span>
      <input
        ref={inputRef}
        id="room-code-input"
      ...
      >
        {Array.from({ length }).map((_, i) => {
          ...
          const caretHere = focused && sel.start === sel.end && sel.start === i;
          const caretAtEnd =
            focused && sel.start === sel.end && sel.start === length && i === length - 1;
          const selected = focused && sel.end > sel.start && i >= sel.start && i < sel.end;
          ...
          return (
            <div
              ...
              className={cn(
                ...
                focused &&
                  !invalid &&
                  i === value.length &&
                  "border-ember/70 shadow-[0_0_0_3px_rgba(201,135,74,0.12)]",
            ...
            >
              {char && (
                <span key={`${i}-${char}`} className="animate-char-in inline-block">
                  {char}
                </span>
              )}
              {caretHere && (
