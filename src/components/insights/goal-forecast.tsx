  ...
  return (
    <section className="border border-white/10 rounded-md p-6 bg-black/30">
      ...
      <div className="mt-3 grid grid-cols-3 gap-3 text-xs text-silver-dim">
        <div>
          Avg / day · <span className="text-silver">{data.avgDailyMinutes}m</span>
        </div>
        <div>
          Weekly ·{" "}
          <span className="text-silver">{Math.round(data.weeklyForecastMinutes / 60)}h</span>
        </div>
        <div>
          Monthly ·{" "}
          <span className="text-silver">{Math.round(data.monthlyForecastMinutes / 60)}h</span>
        </div>
      </div>
