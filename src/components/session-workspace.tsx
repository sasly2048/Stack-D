    ...
    <div className="glass rounded-lg p-4 space-y-3">
      ...
      <ul className="space-y-1.5 max-h-72 overflow-y-auto pr-1">
        ...
        {items.map((i) => (
          <li
            key={i.id}
            className="group flex items-start gap-2 text-xs border border-white/5 rounded px-2 py-1.5 bg-white/[0.02]"
          >
            {i.kind === "todo" ? (
              <input
                type="checkbox"
            ...
            <div className="flex-1 min-w-0">
              {i.kind === "link" && i.url ? (
                ...
                <span
                  className={`break-words ${i.done ? "line-through text-muted-foreground" : "text-silver"}`}
                >
                  {i.content}
                </span>
              )}
