...
export function SessionRecapCard(props: Props) {
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      doc.save(
        `stackd-recap-${safeCode}-${new Date(recap.generatedAt).toISOString().slice(0, 10)}.pdf`,
      );
    ...
    <div className="border border-ember/25 rounded-2xl p-6 sm:p-8 bg-gradient-to-br from-ember/[0.05] via-transparent to-transparent">
      ...
      {loading && !recap ? (
        ...
        <div className="space-y-5">
          ...
          <p className="text-sm text-silver-dim leading-relaxed text-balance">{recap.summary}</p>
