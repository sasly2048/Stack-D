...
export function UserHoverCard({
    ...
    try {
      ...
      setData(
        (prev) =>
          prev && {
            ...prev,
            friendship: { id: "pending", status: "pending", direction: "outgoing" },
          },
      );
    } catch (e) {
  ...
  async function handleAccept() {
    ...
    try {
      ...
      setData({
        ...data,
        friendship: { ...data.friendship, status: "accepted", direction: "friend" },
      });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed");
    } finally {
            ...
            {!data.is_self && (
              <div className="pt-1">
                {data.friendship?.status === "accepted" && (
                  <div className="text-[10px] font-mono uppercase tracking-wider text-ember">
                    ◆ Friends
                  </div>
                )}
