...
export const Route = createFileRoute("/_authenticated/timeline")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Chronological record of your focus sessions with reactions from friends.",
      },
...
function TimelineError({ reset }: { reset: () => void }) {
  const r = useRouter();
  return (
    ...
    <div className="min-h-screen bg-obsidian text-silver">
      ...
      <main className="max-w-3xl mx-auto px-6 pt-24 pb-16 space-y-8">
              ...
              <div className="relative border-l border-white/10 pl-6 pb-6">
                ...
                >
                  ...
                  <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 text-sm">
                    ...
                    <span className="text-silver-dim font-mono">
                      {fmtDuration(s.duration_seconds)}
                    </span>
                    <span className="text-silver-dim font-mono">+{s.xp_earned} XP</span>
                    {s.breaches_count > 0 && (
                      <span className="text-breach font-mono">
                        {s.breaches_count} breach{s.breaches_count > 1 ? "es" : ""}
                      </span>
                    )}
                  </div>
                  {s.notes && (
        ...
        <div className="text-center pt-4">
          <Link
            to="/dashboard"
            className="font-mono text-[10px] tracking-[0.3em] uppercase text-muted-foreground hover:text-ember"
          >
            ← ANALYTICS
          </Link>
        </div>
...
function ProactiveCard({ ai }: { ai: ProactiveInsight }) {
  const riskColor =
    ai.burnout.risk === "high"
      ? "text-breach border-breach/40"
      : ai.burnout.risk === "medium"
