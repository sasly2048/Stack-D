...
Vary the language every time — do not repeat the sample line. Return only the JSON object.`,
      ...
      const cleaned: BrandProse = {
        ...
        philosophyOpener: String(data.philosophyOpener ?? BRAND_FALLBACK.philosophyOpener).slice(
          0,
          320,
        ),
        philosophyClosing: String(data.philosophyClosing ?? BRAND_FALLBACK.philosophyClosing).slice(
          0,
          120,
        ),
  ...
  .handler(async ({ context }): Promise<SessionRecommendation> => {
    ...
    if (rows.length === 0) {
      return {
        ...
        rationale:
          "No history yet. Start light: twenty minutes is long enough to feel the silence, short enough to complete cleanly.",
        confidence: "low",
        basedOnSessions: 0,
        generatedAt: new Date().toISOString(),
      };
    ...
    const avgMin = Math.round(
      rows.reduce((a, r) => a + (r.duration_seconds ?? 0), 0) / rows.length / 60,
    );
...
export const generateDashboardInsights = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<DashboardInsights> => {
    ...
    const last5Avg = last5.length
      ? Math.round(last5.reduce((a, r) => a + (r.score ?? 0), 0) / last5.length)
      : avgScore;
    const prev5Avg = prev5.length
      ? Math.round(prev5.reduce((a, r) => a + (r.score ?? 0), 0) / prev5.length)
      : last5Avg;
...
Voice is obsidian, ceremonial, restrained. No emoji. No exclamation marks.`,
      ...
      return {
        ...base,
        title: cleanRun
          ? `${mins}-Minute Clean Stack`
          : `${mins}-Minute Session · ${data.breachesCount} anomalies`,
