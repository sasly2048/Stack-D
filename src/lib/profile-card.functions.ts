  ...
  .handler(async ({ data, context }): Promise<ProfileCardData> => {
    ...
    const [
      ...
      supabase
        .from("profiles")
        .select(
          "id, display_name, username, avatar_url, bio, title, prestige_level, banner_gradient, banner_url, pinned_showcase, lifetime_xp, current_focus_streak, best_streak, total_focus_seconds, created_at, last_active_at",
        )
        .eq("id", targetId)
        .maybeSingle(),
      supabase
    ...
    const mineSet = new Set(
      (myFriends ?? []).map((r) =>
        idOf(r as { requester_id: string; addressee_id: string }, userId),
      ),
    );
    const theirs = (theirFriends ?? []).map((r) =>
      idOf(r as { requester_id: string; addressee_id: string }, targetId),
    );
    const mutuals = theirs.filter(
      (id) => mineSet.has(id) && id !== targetId && id !== userId,
    ).length;
