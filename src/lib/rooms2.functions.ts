...
export const updateRoomMeta = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.collective_goal_seconds !== undefined)
      patch.collective_goal_seconds = data.collective_goal_seconds;
...
export const promoteModerator = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) =>
    z.object({ roomId: z.string().uuid(), userId: z.string().uuid() }).parse(d),
  )
  .handler(async ({ data, context }) => {
...
export const listRoomEvents = createServerFn({ method: "POST" })
  ...
  .inputValidator((d) =>
    z
      .object({ roomId: z.string().uuid(), limit: z.number().int().min(1).max(100).default(30) })
      .parse(d),
  )
