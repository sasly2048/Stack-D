...
export const listFeed = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }): Promise<{ rows: FeedItem[] }> => {
    ...
    const ids = Array.from(new Set((events ?? []).map((e) => e.user_id))).filter(
      (x) => x !== userId,
    );
      ...
      rows: (events ?? []).map((e) => ({
        ...
        display_name:
          e.user_id === userId
            ...
            : (profs[e.user_id]?.display_name ?? null),
        avatar_url:
          e.user_id === userId ? (me?.avatar_url ?? null) : (profs[e.user_id]?.avatar_url ?? null),
