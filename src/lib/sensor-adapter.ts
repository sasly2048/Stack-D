...
const webAdapter: SensorAdapter = {
  ...
  subscribeOrientation(handler) {
    ...
    const fn = (e: DeviceOrientationEvent) =>
      handler({ beta: e.beta, gamma: e.gamma, alpha: e.alpha });
  ...
  subscribeMotion(handler) {
    ...
    const fn = (e: DeviceMotionEvent) =>
      handler({ accelerationIncludingGravity: e.accelerationIncludingGravity });
  ...
  async requestPermissions() {
    ...
    try {
      if (DME?.requestPermission) await DME.requestPermission();
    } catch {
      /* noop */
    }
  },
...
function createCapacitorAdapter(Motion: typeof import("@capacitor/motion").Motion): SensorAdapter {
  return {
    id: "capacitor",
    subscribeOrientation(handler) {
      const p = Motion.addListener("orientation", (e) =>
        handler({ beta: e.beta, gamma: e.gamma, alpha: e.alpha }),
      );
      return () => {
    ...
    subscribeMotion(handler) {
      const p = Motion.addListener("accel", (e) =>
        handler({ accelerationIncludingGravity: e.accelerationIncludingGravity ?? null }),
      );
      return () => {
        p.then((h) => h.remove()).catch(() => {});
      };
