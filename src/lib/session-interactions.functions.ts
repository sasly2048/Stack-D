...
export const listTimeline = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<TimelineSession[]> => {
    ...
    let q = supabase
      .from("focus_history")
      .select(
        "id, created_at, duration_seconds, score, tier, xp_earned, breaches_count, notes, tags, room_id",
      )
    ...
    return sessions.map((s) => ({
      ...s,
      reactions: Array.from((grouped.get(s.id) ?? new Map()).entries()).map(([emoji, v]) => ({
        emoji,
        count: v.count,
        mine: v.mine,
      })),
